...
Each subdirectory has a **`README.md`** with **initial setup**; stacks that use SOPS also get decrypted files under **`docker/<stack>/`** (see below).

### Secrets (`sops-nix`)

Docker env files (`docker/honcho`, `docker/calendar`, `docker/jira`, `docker/kdbx`) and calendar-related JSON are produced by **Home Manager** from SOPS-encrypted YAML under **`secrets/`**:

- [`secrets/default.yaml`](../secrets/default.yaml) — Honcho (`honcho_env` → `docker/honcho/.env`)
- [`secrets/calendar.yaml`](../secrets/calendar.yaml) — Calendar stack: **`radicale_env`** → **`~/.calendar/radicale/.env`**, **`chronos_accounts_json`** → **`~/.calendar/chronos/accounts.json`**, **`google_oauth_client_json`** → **`~/.calendar/google/oauth.json`** (see [`docker/calendar/README.md`](calendar/README.md))
